// 1. Select DOM elements correctly
const RecipesContainer = document.querySelector('#recipe-container');
const input = document.querySelector('#search');
const button = document.querySelector('#search-btn'); // Matches the ID in the updated HTML
...
function search() {
    ...
    sortedRecipes.forEach(renderRecipes);
}
...
function recipesTemplate(recipe) {
    ...
    <article class="recipe-card">
        <img src="${recipe.image}" alt="Image of ${recipe.name}" class="recipe-photo">
        <div class="recipe-descriptions">
            <p class="type-of-food">${recipe.tags[0]}</p>
            <h4>${recipe.name}</h4>
